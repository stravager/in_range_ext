...
#define IN_RANGE_EXT_ASSERT(expr) \
		(void)(!!(expr) ? 0 : \
	...
	namespace detail {
		namespace constexpr_cmath {
			...
			constexpr bool signbit(F f)
			{
				...
				return f < 0 ||
					(f == 0 && std::bit_cast<fbytes>(f) == std::bit_cast<fbytes>(-F(0)));
#endif
			}
		...
		template<int radix, int num_digits>
		struct decomp_rep
		{
			int category{};
			bool signbit{};
		...
		// Utilities.
		template<int radix, int num_digits>
		constexpr bool isnan(const decomp_rep<radix, num_digits>& x) { return x.category == FP_NAN; }

		template<int radix, int num_digits>
		constexpr bool isinf(const decomp_rep<radix, num_digits>& x) { return x.category == FP_INFINITE; }

		template<int radix, int num_digits>
		constexpr bool iszero(const decomp_rep<radix, num_digits>& x) { return x.category == FP_ZERO; }

		template<int radix, int num_digits>
		constexpr bool ispos(const decomp_rep<radix, num_digits>& x) { return !x.signbit && !isnan(x) && !iszero(x); }

		template<int radix, int num_digits>
		constexpr bool isneg(const decomp_rep<radix, num_digits>& x) { return x.signbit && !isnan(x) && !iszero(x); }

		template<int radix, int num_digits>
		constexpr bool isposinf(const decomp_rep<radix, num_digits>& x) { return !x.signbit && isinf(x); }

		template<int radix, int num_digits>
		constexpr bool isneginf(const decomp_rep<radix, num_digits>& x) { return x.signbit && isinf(x); }

		// "less" comparator.
		template<int radix, int num_digits>
		constexpr bool operator<(const decomp_rep<radix, num_digits>& lhs, const decomp_rep<radix, num_digits>& rhs)
		{
		...
		template<int radix, integer I>
		constexpr int count_digits(I i)
		{
		...
		// (Conversion TO intger is not supported and not needed here).
		template<std::floating_point F>
		class decomp
		{
		...
		public:
			...
			// Conversion from F.
			constexpr explicit decomp(F f)
				: rep{
					.category = constexpr_cmath::fpclassify(f),
					.signbit = constexpr_cmath::signbit(f),
					.ilogb = constexpr_cmath::ilogb(f)
				}
			{
				// Need extra handling below if fpclassify can return any non-standard values.
				IN_RANGE_EXT_ASSERT(
					rep.category == FP_ZERO ||
					rep.category == FP_SUBNORMAL ||
					rep.category == FP_NORMAL ||
